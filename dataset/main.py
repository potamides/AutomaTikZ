...
if __name__ == "__main__":
    ...
    good = text2tikz.filter(lambda ex: ex['origin'] in MODELS or len(tokenize(ex['caption'])) >= MIN_CAPTION_LENGTH)
    bad = text2tikz.filter(lambda ex: ex['origin'] in MODELS or len(tokenize(ex['caption'])) < MIN_CAPTION_LENGTH)
    ...
    text2tikz.to_parquet("text2tikz-raw.parquet", compression="GZIP") # type: ignore
    train.to_parquet("text2tikz-train.parquet", compression="GZIP") # type: ignore
    test.to_parquet("text2tikz-test.parquet", compression="GZIP") # type: ignore
